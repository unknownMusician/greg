...
{
    public sealed class RelativeDeterminismWindow : EditorWindow
    {
        private Dictionary<(Type, Type), HashSet<Type>> missingOrderings;
        private int snapshot = -1;
        private Vector2 missingOrderingsScroll;
        
        [MenuItem("Are You Fruits?/Relative Determinism")]
        private static void Create()
        {
            EditorWindow.GetWindow<RelativeDeterminismWindow>("Relative Determinism");
        }
        ...
        private void OnGUI()
        {
            if (snapshot != HandlersOrderingDeterminismInfoHolder.Snapshot)
            {
                ...
                missingOrderings = Utils.GetMissingRelativeOrderings(neededOrderings);
            }
